...
layui.define("view", function (e) {
    ...
    const mode = 'admin';
    const App = $('#LAY_app');
    const FlexibleIcon = $('#LAY_app_flexible');
    const classSpreadLeft = "layui-icon-spread-left";
    const classShrinkRight = "layui-icon-shrink-right";
    const classSideSpreadSM = "layadmin-side-spread-sm";
    const classSideShrink = "layadmin-side-shrink";
    ...
    const P = {
        ...
        // tabsPage: {},
        // resizeFn: {},
        screen: function () { // 判断window宽度 返回对应数值  1200以上 => 3  992以上 => 2  768以上 => 1  768以下 => 0
            const screenWidth = $(window).width();
            return screenWidth >= 1200 ? 3 : screenWidth >= 992 ? 2 : screenWidth >= 768 ? 1 : 0
        },
        ...
        sideFlexible: function (status) {
            const screenNum = P.screen();
            if ("spread" === status) {
                FlexibleIcon.removeClass(classSpreadLeft).addClass(classShrinkRight);
                screenNum < 2 ?
                    App.addClass(classSideSpreadSM) :
                    App.removeClass(classSideSpreadSM);
                App.removeClass(classSideShrink);
            } else {
                FlexibleIcon.removeClass(classShrinkRight).addClass(classSpreadLeft);
                screenNum < 2 ?
                    App.removeClass(classSideShrink) :
                    App.addClass(classSideShrink);
                App.removeClass(classSideSpreadSM);
            }
            // layui.event.call(this, mode, "side({*})", {
            //     status
            // })
        },
        // escape: function (e) {
        ...
        on: function (event, callback) {
            return layui.onevent.call(this, mode, event, callback)
        },
    ...
    const F = P.events = {
        flexible: function () {
            const bool = FlexibleIcon.hasClass(classSpreadLeft);
            P.sideFlexible(bool ? "spread" : null)
        },
        refresh: function () {
